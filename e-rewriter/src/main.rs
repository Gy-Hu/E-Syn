...
pub struct Mixcost;
    impl CostFunction<Prop> for Mixcost{
                ...
                C: FnMut(Id) -> Self::Cost,
            {
                ...
                let costdepth = 1 + enode.fold(0, |max, id| max.max_random(costs(id)));
                //let result = 0.2*costsize+(costdepth/5*4)as f64;
...
fn main() ->Result<(), Box<dyn std::error::Error>> {
    ...
    let mut res_cost: HashMap<i32, i32> = HashMap::new();
    ...
    for min_key in min_keys.iter() {
        ...
        // 创建文件名，可以根据需要自定义
        let output_file_name = format!("output_{}.txt", count);
